...
public class JedisPoolTest extends Assert {
    ...
    public void securePool() {
        ...
        JedisPool pool = new JedisPool(config, hnp.host, hnp.port, 2000,"bestwiz");
	...
	j.select(1);
	j.set("foo", "bar");
	JedisPool pool = new JedisPool("redis://:bestwiz@"+hnp.host+":"+hnp.port+"/1");
	Jedis jedis = pool.getResource();
	...
	j.select(2);
	j.set("foo", "bar");
	JedisPool pool = new JedisPool(new URI("redis://:bestwiz@"+hnp.host+":"+hnp.port+"/2"));
	Jedis jedis = pool.getResource();
	assertEquals("PONG", jedis.ping());
	assertEquals("bar", jedis.get("foo"));
    }
